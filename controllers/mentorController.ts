...
const getMentor = (
  ...
  Mentor.find()
    .then((mentors: IMentor[]) => {
      ...
      User.findOne({ email: arr[0].mentoringUser[0].email })
        .then((user) => {
          if (!user) {
            throw new http500Error("No user found");
          }
...
const updateMentor = (
  ...
  Mentor.findByIdAndUpdate(req.params.id, req.body, { new: true })
    .then((mentor: any) => {
      User.findOne({ _id: mentor.mentoringUserId })
        .then((user) => {
          if (!user) {
...
const deleteMentor = (
  ...
  Mentor.findOneAndDelete({ _id: req.params.id })
    .then((mentor: any) => {
      Task.deleteMany({ mentorId: mentor.mentorId });
      res.status(204);
    })
    .catch((err) => {
      next(err);
